  ...
  , Timer(io)
{
  ...
  node = Node(Server, ObjectId::Server_ServerDiagnostics_EnabledFlag);
  node.SetValue((bool) false);
  node = Node(Server, ObjectId::Server_ServiceLevel);
  node.SetValue((uint8_t)255);

  node = Node(Server, ObjectId::Server_ServerStatus_BuildInfo_SoftwareVersion);
  node.SetValue(std::string("1.2"));
  node = Node(Server, ObjectId::Server_ServerStatus_BuildInfo_BuildDate);
  node.SetValue(DateTime::Current());

  node = Node(Server, ObjectId::Server_ServerCapabilities_ServerProfileArray);
  node.SetValue(std::vector<std::string>{"http://opcfoundation.org/UAProfile/Server/StandardUA"});
