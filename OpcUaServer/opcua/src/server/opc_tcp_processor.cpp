  ...
  */
  ...
  switch (message)
    ...
    case CREATE_SESSION_REQUEST:
    {
      ...
      AuthenticationToken = ExpandedNodeId(789910933, 0);
      response.Parameters.SessionId = SessionId;
      response.Parameters.AuthenticationToken = AuthenticationToken;
    ...
    case ACTIVATE_SESSION_REQUEST:
    {
      ...
      if (SessionId.NumericData.Identifier == 0)
      {
          ServiceFaultResponse faultResponse;
          FillResponseHeader(requestHeader, faultResponse.Header);
          faultResponse.Header.ServiceResult = StatusCode::BadSessionIdInvalid;

          SecureHeader secureHeader(MT_SECURE_MESSAGE, CHT_SINGLE, ChannelId);
          secureHeader.AddSize(RawSize(algorithmHeader));
          secureHeader.AddSize(RawSize(sequence));
          secureHeader.AddSize(RawSize(faultResponse));
          ostream << secureHeader << algorithmHeader << sequence << faultResponse << flush;
